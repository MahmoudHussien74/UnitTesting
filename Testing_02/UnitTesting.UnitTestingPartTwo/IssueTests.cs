...
using UnitTestingPartTwo.Exceptions;

namespace UnitTesting.UnitTestingPartTwo;
...
public class IssueTests
{
    ...
    public void Constructor_WithIssueNotProvidingCreatedAt_ReturnsCreatedAtCurrentDateTime()
    {
        ...
        var arr = new Issue("#1", Priority.Low, Category.Hardware, null);

        var actual = arr.CreatedAt;



        Assert.InRange(actual, before, DateTime.Now);
       ...
       (string desc, Priority priority, Category category, string createdAt, string expected)
    {
        ...
        Assert.Equal(actual.Substring(0, 10), expected.Substring(0, 10));
